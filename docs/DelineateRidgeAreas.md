...
This section demonstrates how ScrollStats delineates ridge areas from an input DEM.
...
The steps above to create what's called the *ridge area raster* are all performed in the two following convenience functions:
...
>NOTE: this section breaks down how scrollstats accomplishes the 4 main steps listed at the top of the page. However, this is just for illustration. The convenience functions described above should be used to create the ridge area raster.
...
Both of these transformations can be classified into "ridge" and "not ridge" areas by applying a threshold at 0, creating a binary raster. Using two independent classifications to identify ridge areas, we can be confident that the union of these two binary rasters represent the true ridge locations within the DEM.
...
First, apply a threshold at 0 to each of the transform rasters. If the pixel value is greater than 0, assign it a value of 1 (ridge). If the value is less than or equal to 0, assign it a value of 0 (not-ridge)
...
Then clip the agreement raster with the bend polygon. This greatly reduces the number of features before denoising the output in step 4.

```python 
bend = gpd.read_file(bend_path)
bend_geom = bend.loc[0, "geometry"]

# Clip the DEM and agreemnt raster by assigning all pixels outside the bend geometry to np.nan
...
Apply binary opening and closing operations to remove very small (approximately single pixel width) features then remove all other small features with a contiguous pixel count less than a given size.
...
[Create Vector Datasets](CreateVectorDatasets.md) will walk through the processes and rationale for creating the vector datasets needed for ScrollStats. The ridge area raster created from this process will be used later.
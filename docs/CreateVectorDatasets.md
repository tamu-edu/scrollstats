...
ScrollStats uses the following vector datasets for a bend in order to calculate ridge metrics:
...
While all of these above datasets are used, not all are strictly necessary, not all are required at the beginning of the workflow, and some are generated from ScrollStats. 
...
smooth_ridges.plot(ax=ax, color="red", lw=1.5, alpha=0.6, zorder=3, label="Smoothed Ridges")
...
All vector datasets needed for ScrollStats are now created. [Calculate Ridge Metrics](CalculateRidgeMetrics.md) will contain the example code to calculate ridge metrics from the ridges generated in this notebook and the ridge area raster created in [Delineate Ridge Areas](DelineateRidgeAreas.md)
